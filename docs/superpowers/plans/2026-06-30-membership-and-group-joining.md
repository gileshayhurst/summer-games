...
**Goal:** Add Sign in with Apple, a real membership model (group_members table), public/private group visibility, a join-by-code/link flow, a public group directory, and role-based admin access — replacing the existing 4-digit PIN gate.

**Architecture:** Supabase Auth (Apple provider) supplies identity. A new `profiles` table (auto-created by DB trigger) and `group_members` join table store who belongs to which groups. App-layer guards in `lib/auth.ts` enforce visibility and role requirements in every page layout and API route. All existing game logic and tables are unchanged.

**Tech Stack:** Next.js 14 App Router, `@supabase/ssr`, Supabase Auth (Apple), Tailwind CSS, TypeScript

---

## ⚠️ Manual prerequisites (do before Task 1)

1. **Apple Developer account** — You need an Apple Services ID, Team ID, Key ID, and a downloaded `.p8` private key. Follow: https://supabase.com/docs/guides/auth/social-login/auth-apple
2. **Enable Apple provider in Supabase** — Dashboard → Authentication → Providers → Apple → enter credentials → Save.
3. **Add Authorized Redirect URL in Apple** — In your Apple Services ID config, add: `https://<your-supabase-project>.supabase.co/auth/v1/callback`
...
git commit -m "feat: add auth callback route for Apple OAuth"
```
...
git commit -m "feat: add sign-in page with Apple OAuth"
```
